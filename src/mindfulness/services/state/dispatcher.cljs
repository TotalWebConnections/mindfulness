(ns mindfulness.services.state.dispatcher
  (:require [mindfulness.services.state.global :refer [app-state update-active-view update-home-view update-flow-view update-individual-entry update-stats-view hide-tutorial]]
...
(defmethod handle-state-change "hide-tutorial"
  [action]
  (hide-tutorial app-state (:value action)))
